...
def find_books_by_genre_and_language(genre, language):
    result = Book.objects.filter(genre=genre, language=language)
    return result


def find_authors_nationalities():
    result = []
    nationalities = Author.objects.exclude(nationality=None)
    for n in nationalities:
        result.append(f"{n.first_name} {n.last_name} is {n.nationality}")
...
def order_books_by_year():
    ...
    for book in books:
        info.append(f"{book.publication_year} year: {book.title} by {book.author}")

    return '\n'.join(info)


def delete_review_by_id(review_id):
    result = Review.objects.get(pk=review_id)

    result.delete()

    return f"Review by {result.reviewer_name} was deleted"
...
def filter_authors_by_nationalities(nationality):
    ...
    for author in authors:
        if not author.biography:
            author_biography.append(f"{author.first_name} {author.last_name}")
        else:
            author_biography.append(f"{author.biography}")

    return '\n'.join(author_biography)
